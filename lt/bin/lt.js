...
(async () => {
  ...
  let tunnelURL = tunnel.url || tunnel.cachedUrl;
  console.log(tunnelURL);
  tunnelURL = tunnelURL.replace('https', 'http');
  const content = 'auth-server-url=' + tunnelURL + '/auth' + '\n' + 'issuer-uri='+ tunnelURL + '/auth/realms/3cn\n'


  //write auth-server-url=tunnelURl in .evn file in the root directory for our project
  const fs = require('fs');
  fs.appendFile('../.env', content, (err) => {
    if(err) throw err;
    console.log('.evn saved!');
  })
